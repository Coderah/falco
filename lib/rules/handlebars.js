...
module.exports = function handlebarRules(l) {
    ...
    l.addRule(/\{\{\/([\w\.]+)\}\}/, function(match, helperName) {
        ...
        return { type: 'endExpressionBlock', helperName: helperName };
    }, ['inExpressionBlock']);

    l.addRule(/\{\{else\}\}/, function(match) {
        return { type: 'elseExpression' };
    }, ['inExpressionBlock']);
    ...
    l.addRule(/(\{{2,3})(#)?(?:([\w\-]+)\s)?((?:[\S\/\.]+?\s?)+?)(\}{2,3})/,
        function(match, opening, modifier, helperName, expressionArguments, closing) {
            ...
            if (opening.length !== closing.length) {
                return {
                    type: 'warning',
                    message: 'ignoring broken falco expression (doesn\'t have matching opening and ending braces)',
                    match: match,
            ...
            if (modifier === '#') {
                this.setState('inExpressionBlock');

                details.type = 'openExpressionBlock';
            } else {
